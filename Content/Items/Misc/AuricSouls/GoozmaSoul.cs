...
{
    public class GoozmaSoul : ModItem
    {
        ...
        public override void Update(ref float gravity, ref float maxFallSpeed)
        {
            if (heartbeatSound == null)
                heartbeatSound = new LoopingSound(AssetDirectory.Sounds.Souls.GoozmaSoulHeartbeat, new HUtils.ItemAudioTracker(Item).IsActiveAndInGame);
            heartbeatSound.PlaySound(() => Item.position, () => 1f, () => 0f);

            if (droneSound == null)
                droneSound = new LoopingSound(AssetDirectory.Sounds.Souls.GoozmaSoulDrone, new HUtils.ItemAudioTracker(Item).IsActiveAndInGame);
            droneSound.PlaySound(() => Item.position, () => 1.5f, () => 0f);

            if (breathSoundCounter-- <= 0) {
                SoundEngine.PlaySound(AssetDirectory.Sounds.Souls.GoozmaSoulBreathe, Item.Center);
